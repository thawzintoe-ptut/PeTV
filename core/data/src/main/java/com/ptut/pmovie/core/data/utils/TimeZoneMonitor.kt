...
import com.ptut.pmovie.core.common.di.ApplicationScope
    ...
    constructor(
        ...
        override val currentTimeZone: SharedFlow<TimeZone> =
            callbackFlow { // Send the default time zone first.
                        ...
                        override fun onReceive(
                            ...
                            val zoneIdFromIntent =
                                if (VERSION.SDK_INT < VERSION_CODES.R) {
                                    null
                                } else { // Starting Android R we also get the new TimeZone.
                                    intent.getStringExtra(Intent.EXTRA_TIMEZONE)
                                        ?.let { timeZoneId -> // We need to convert it from java.util.Timezone to java.time.ZoneId
                                            val zoneId =
                                                ZoneId.of(
                                                    timeZoneId,
                                                    ZoneId.SHORT_IDS,
                ...
                .flowOn(ioDispatcher) // Sharing the callback to prevent multiple BroadcastReceivers being registered
